...
export class StreamChunkSizerService  extends Transform {
  ...
  constructor(private readonly targetChunkSizeInBytes: number = STREAM_CHUNK_SIZE) {
    super();
  }

  _transform(chunk: any, encoding: BufferEncoding, callback: TransformCallback): void {
    const binaryBuffer = Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk, encoding);
    ...
    while (this.currentSize >= this.targetChunkSizeInBytes) {
      ...
      const chunkToRelease = totalBuffer.subarray(0, this.targetChunkSizeInBytes);
